...
function kill(id, colorcurr) {
  ...
  if (colordest == "n") return false;
    ...
    if (limit.getAttribute("value") == wking) blackwins = 1;
    if (limit.getAttribute("value") == bking) whitewins = 1;
    {
      return true;
    }
  } else return false;
...
function knightmove(curr, color) {
  ...
  for (let index = 0; index < nid.length; index++)
    if (valid(nid[index], color) || kill(nid[index], color))
      nextid.push(nid[index]);
  if (!disable) nextid = kingscheck(nextid, color);
  return nextid;
}
...
function wpawnmove(curr) {
  ...
  if (parseInt(curr / 10) == 2 && valid(curr + 20, color))
    nextid.push(curr + 20);
  if (
    10 < curr + 11 &&
    (curr + 11) % 10 != 0 &&
...
function bpawnmove(curr) {
  ...
  color = "b";
  if (valid(curr - 10, color)) nextid.push(curr - 10);
  if (parseInt(curr / 10) == 7 && valid(curr - 20, color))
    nextid.push(curr - 20);
  if (
    10 < curr - 11 &&
    (curr - 11) % 10 != 0 &&
...
function kingscheck(ids, color) {
  ...
  for (let x = 0; x < path.length; x++) {
    ...
    if (
      danger.getAttribute("value") == bishop ||
...
let wkingsplace = document.getElementById("15"),bkingsplace = document.getElementById("85");
while (!(blackwins||whitewins)) {
      ...
      case "white":
        {
          ...
          for (let index = 0; index < allowedmoves.length; index++) {
            let box = eval(
              "document.getElementById(" + allowedmoves[index] + ")"
            );
